...
@router.get("/exercises", response_model=List[Exercise], summary="R̩cup̩ration du catalogue d'exercices")
async def get_exercises(category: Optional[str] = Query(None, description="Filtrer par cat̩gorie")):
    """
    R̩cup̬re la liste des exercices disponibles (depuis Supabase ou catalogue local fallback).
    """
    if category:
...
@router.post("/exercises/filter", response_model=FilterExercisesResponse, summary="Filtrage strict par ̩quipement, blessures et Readiness")
async def filter_exercises(payload: FilterExercisesRequest):
    """
    Applique les HARD Rules pour exclure les exercices incompatibles avec l'̩quipement,
    les blessures d̩clar̩es ou le statut de fatigue.
    """
...
@router.post("/exercises/seed", summary="Injection du catalogue par d̩faut dans Supabase")
async def seed_exercises():
    """
    Injecte les exercices de base (MMA, Striking, Plyo, Mobilit̩) dans la table 'exercises' de Supabase.
    """
